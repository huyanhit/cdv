<script setup>
    ...
    onBeforeMount(()=>{
        // socket.emit(AppConst.APP_JOIN_CHANNEL, AppConst.APP_JOIN_USER + '1');
        store.dispatch('sync_data', {CURRENT_USER:{id:'1'}});
        store.dispatch('sync_data', {EMOIJ:{'1': IconConst.EMOIJ}});
...
export default {
    sockets: {
        ...
        user_update_member(data){
            this.$store.dispatch('sync_data', data.sync);
        },
        room_push_message(data){
            this.$store.dispatch('push_list', data.push);
            this.$store.dispatch('sync_data', data.sync);
        ...
        room_to_all(data){
            ...
            this.$store.dispatch('sync_data', { 'MEMBER': {[key]: { mention: this.MEMBER[key].mention + 1 }}});
        },
        user_add_room(data){
            this.$store.dispatch('push_list', data.push);
            this.$store.dispatch('sync_data', data.sync);
            this.$socket.emit(AppConst.APP_JOIN_CHANNEL, AppConst.APP_JOIN_ROOM + data.resp);
        }
    }
}
</script>